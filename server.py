...
def build_router():
    ...
    return Router(
        ...
        max_retries=int(env.get("ROUTER_MAX_RETRIES", "1")),
    )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, status, payload, content_type="application/json; charset=utf-8"):
        ...
        self.send_header("Access-Control-Allow-Origin", os.getenv("ROUTER_CORS_ORIGIN", "*"))
        self.end_headers()
    ...
    def do_GET(self):
        ...
        if path == "/api/health":
            return self._send(200, {"ok": True, "providers": len(ROUTER.providers()), "version": "web-v3"})
        if path == "/api/skills":
            return self._send(200, {"skills": ROUTER.list_skills()})
        if path == "/api/conversations":
            return self._send(200, {"conversations": STORE.list_conversations()})
        if path.startswith("/api/conversations/"):
            item = STORE.get_conversation(path.rsplit("/", 1)[-1])
            return self._send(200, item) if item else self._send(404, {"error": "Conversation not found"})
        if path == "/api/analytics":
            return self._send(200, STORE.analytics())
    ...
    def do_DELETE(self):
        ...
        if self._rate_limited():
            return self._send(429, {"error": "Too many requests"})
        if path.startswith("/api/conversations/"):
            STORE.delete_conversation(path.rsplit("/", 1)[-1])
    ...
    def do_POST(self):
        ...
        try:
            ...
            if conversation_id:
                title = next((m.get("content", "New chat") for m in messages if m.get("role") == "user"), "New chat")
                STORE.save_conversation(
                    conversation_id,
                    title,
                    messages + [{"role": "assistant", "content": result.get("content", "")}],
                )
    ...
    def _static(self, path):
        ...
        try:
            ...
            return self._send(200, data, mimetypes.guess_type(str(target))[0] or "application/octet-stream")
        except OSError:
            return self._send(404, {"error": "Not found"})
