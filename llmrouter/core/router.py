...
class Router:
    ...
    def stream(self, messages, model=None, temperature=None, max_tokens=None):
        """Yield incremental events while retaining provider failover before output starts."""
        failures = []
        for p in self._providers:
            if not p.available() or self._over_limit(p.name): continue
            attempts = 0
            while attempts <= self.max_retries:
                try:
                    ...
                    for event in streamer:
                        ...
                        usage = event.get("usage") or usage; selected_model = event.get("model") or selected_model
                        yield event
